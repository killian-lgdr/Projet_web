<?php
    ...
    class PDManager extends Manager
    {
        public function getDelegue($nom, $prenom)
        {
            ...
            {
                $db = $this->dbConnect();
                $req = $db->query('SELECT nom_delegue, prenom_delegues, localisation.nom_localisation from delegue inner join localisation on delegue.ID_Localisation = localisation.ID_localisation where pilote.nom_pilote = '.$_GET['nom_del'].' AND pilote.prenom_pilote = '.$_GET['prenom_del']);
                return $req;
            }
        }
